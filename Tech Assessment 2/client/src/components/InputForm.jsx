    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          console.error('Geolocation fetch error:', err.response?.data || err.message);
          setError(
            err.response?.status === 404
              ? 'Location not found'
              : err.response?.status === 400
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 flex flex-col items-center gap-6">
      <input
        ...
        className="p-3 border rounded-xl w-full max-w-md focus:outline-none focus:ring-2 focus:ring-blue-400"
      />
